...
- [ ] **3.1** Build dashboard layout: top bar (logo, display name, settings, logout).
- [ ] **3.2** Quick stats row: this week’s P&L, win rate (this week), current streak.
- [ ] **3.3** Calendar heatmap (month view): green = profit day, red = loss day, gray = no trade; click day to view/edit.
- [ ] **3.4** “Log Today’s Trade” button — opens trade entry modal.
- [ ] **3.5** Trade entry modal: Date, Number of Trades, Net P&L, Charges (optional), Capital Deployed (optional), Note (max 280). Validation per PRD §2.4. Save/Update/Delete with confirmation for delete.
- [ ] **3.6** Recent entries list: last 7 days with date, P&L, # trades, “Generate Card” button per row.
